function [p,F,pg,evalHist,diversity] = ACuckoov3(K, NestI, S, vardef,Desired_SM)
...
MinNests = 4; % Minimal number of nests
...
Fi = zeros(NoNests,1);
%...and for the next nest positions and fitnesses
piNext = zeros(NoNests,NoDim);
FiNext = zeros(NoNests,1);
ptemp = zeros(1,NoDim);
pmean = zeros(1,NoDim);
...
[numObjEval,Fi] = evaluate_all(pi,Fi,f,NoNests,Desired_SM);
numObj = numObj + numObjEval;
...
while G<K
    ...
    for i = 1:NoDiscard
        ...
        if and(eq(S.constrain,1),or(any(upper),any(lower)))
            ...
            for j = 1:NoDim
                ptemp(1,j) = max(ptemp(1,j),vardef(1,j));
                ptemp(1,j) = min(ptemp(1,j),vardef(2,j));
            end
            piNext(NoNests - i + 1, :) = ptemp;
        else
    ...
    %Now evaluate and update
    [numObjEval,FiNext] = evaluate_all(piNext,FiNext,f,NoNests,Desired_SM);
    numObj = numObj + numObjEval;
    for i = 1:NoDiscard
        pi(NoNests - i + 1,:) = piNext(NoNests - i + 1,:);
        Fi(NoNests - i + 1,:) = FiNext(NoNests - i + 1,:);
    end
    for C = 1:NoTop
        randNest = round(1 + (NoNests-1).*rand(1));
        if (Fi(randNest,1)>FiNext(C,1))
            Fi(randNest,1)=FiNext(C,1);
            pi(randNest,:)=piNext(C,:);
        end
    end

    
     %2a) Emptying routine from yang and deb
    piNext=empty_nests(pi,pa);
    [numObjEval,FiNext] = evaluate_all(piNext,FiNext,f,NoNests,Desired_SM);
    numObj = numObj + numObjEval;
...
function [numObjEval,FiNext] = evaluate_all(piNext,FiNext,f,NoNests,Desired_SM)

    %This function evaluates the objective function for each of the nests in
    %piNext

    
    numObjEval = NoNests;
    
    [ FiNext ] = solveAll( piNext,NoNests,Desired_SM );
